  ...
  return (
    <>
      <Box className="chart-creation">
        <h2>{title}</h2>
        <Button style={{marginBottom:"10px"}} onClick={handleSave} variant="outlined" disabled={isSaving}>
          {isSaving ? "Salvataggio..." : "Salva"}
        </Button>

        <Grid container>
          <Grid size={12}>
            <TextField value={chartTitle || ''} onChange={(e)=>{setChartTitle(e.target.value)}} label="Chart Title" variant="outlined" size="small" />
          </Grid>
